...
namespace MZ {
	...
	{
		VMeshId meshID = createMesh(vertices, indices);
		VShaderId shaderID = createShader(vertShaderFilePath, fragShaderFilePath);
		objects.resize(numObjects+1);
	...
	VMeshId Renderer::createMesh(std::vector<Vertex>* vertices, std::vector<uint32_t>* indices)
	{
		meshes.resize(numMeshes + 1);
	...
	VShaderId Renderer::createShader(std::string vertShaderFilePath, std::string fragShaderFilePath)
	{
		shaders.resize(numShaders + 1);

		createDescriptorSetLayout(shaders[numShaders].descriptorSetLayout);
		createGraphicsPipline(vertShaderFilePath, fragShaderFilePath, shaders[numShaders].pipelineLayout, shaders[numShaders].graphicsPipeline, shaders[numShaders].descriptorSetLayout);
